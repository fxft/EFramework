 ...
 * 随机数据
 */
#define MJRandomData @{@"title":@"text",@"detail":[NSString stringWithFormat:@"随机数据---%d", arc4random_uniform(1000000)]}//[NSString stringWithFormat:@"随机数据---%d", arc4random_uniform(1000000)]
...
{
    // 1.添加假数据
    for (int i = 0; i<5&&self.data.count<15; i++) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self configImage:cell paras:self.data[indexPath.row]];
    
    return cell;
}
- (void)configImage:(UITableViewCell*)cell paras:(NSDictionary*)dic{
    NSString *title = [dic valueForKey:@"title"];
    NSString *detail = [dic valueForKey:@"detail"];
    ;
    cell.textLabel.text = [NSString stringWithFormat:@"%@ %@",title,detail];
    UIButten *imagev = (id)cell.accessoryView;
    if (imagev==nil) {
        imagev = [[[UIButten alloc]init] EFFrame:CGRectMake(0, 0, 40, 40)];
        cell.accessoryView = imagev;
    }
    LogTimeCostBegin(1)
    id image = @"http://202.100.180.20/api.do?getimg&id=123";//[dic valueForKey:@"img"];
//    if (image==nil||([image isKindOfClass:[NSString class]]&&![(NSString*)image notEmpty])) {
//        image = [UIImage imageNamed:@"picture_upload.png"];
//        imagev.userInteractionEnabled = NO;
//    }else if ([image isKindOfClass:[NSString class]]&&[(NSString*)image notEmpty]){
//        image = IMGURLFORID(image);
//        imagev.userInteractionEnabled = NO;
//    }
    LogTimeCostEndTo(2, 1, @"获取图片耗时")
    [imagev EFImage:image];
    imagev.userInfo = image;
    imagev.tagString = @"weightnum";
    LogTimeCostEndTo(3, 2, @"设置图片耗时")
}
