...
@implementation HMCPUMemoryInfo
...
+ (HMMemoryInfo *)memoryInfo {
    ...
    struct task_basic_info _info;
    mach_msg_type_number_t size = sizeof(_info);
    kern_return_t kerr = task_info(mach_task_self(), TASK_BASIC_INFO, (task_info_t)&_info, &size);
    if (kerr == KERN_SUCCESS) {
        info.used = _info.resident_size;
    }
...
+ (HMCPUMemoryInfo *)cpuMemoryInfo{
    static HMCPUMemoryInfo *info = nil;
    if (!info) {
        info = [HMCPUMemoryInfo new];
        info.total = [[NSProcessInfo processInfo] physicalMemory];
    }
    
    info.usage = [self cpuUsage];
    struct task_basic_info _info;
    mach_msg_type_number_t size = sizeof(_info);
    kern_return_t kerr = task_info(mach_task_self(), TASK_BASIC_INFO, (task_info_t)&_info, &size);
    if (kerr == KERN_SUCCESS) {
        info.used = _info.resident_size;
        info.virtuals = _info.virtual_size;
        info.userTime = (NSTimeInterval)_info.user_time.seconds + (NSTimeInterval)_info.user_time.microseconds/1000;
        info.systemTime = (NSTimeInterval)_info.system_time.seconds + (NSTimeInterval)_info.system_time.microseconds/1000;;
    }
...
//    }
    ...
    return curentNetInfo;
}

+ (HMNetworkInfo *)lastNetworkInfo{
    return lastNetInfo;
}

+ (HMNetworkInfo *)diffNetworkInfo{
    return diffNetInfo;
}
