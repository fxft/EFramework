...
+ (BOOL)touch:(NSString *)path ignoreName:(BOOL)ignore{
    ...
    if (ignore) {
        pathName = [path stringByDeletingLastPathComponent];
    }
...
+ (BOOL)touchFile:(NSString *)path{
    ...
    [self touch:path ignoreName:YES];
    
    if ( NO == [[NSFileManager defaultManager] fileExistsAtPath:pathName] )
    {
        return [[NSFileManager defaultManager] createFileAtPath:pathName contents:nil attributes:nil];
    }
