...
- (id)init
{
	...
	if ( self )
	{
        ...
        self.webApiCachePath = [self.cachePath stringByAppendingString:@"HMWebAPI/"];
        self.imagesCachePath = [self.cachePath stringByAppendingString:@"images/"];
	}
	return self;
}
...
- (void)dealloc
{
	...
	self.cachePath = nil;
    self.userDir= nil;
    self.webApiCachePath = nil;
    self.imagesCachePath = nil;
	HM_SUPER_DEALLOC();
}
...
- (BOOL)moveItemForKey:(NSString *)key atBranch:(NSString *)at toBranch:(NSString *)to{
    ...
    BOOL ret = NO;
    @autoreleasepool {
        ret = [[NSFileManager defaultManager] moveItemAtPath:[self fileNameForKey:key branch:at] toPath:[self fileNameForKey:key branch:to] error:Nil];
    }
