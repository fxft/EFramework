...
- (void)loadImageIfNotExist:(BOOL)animated{
    if (self.photo.webUrl) {
        ...
        NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:[self.photo.webUrl stringByAddingPercentEscapesForURL]]];
        
        [self.imageView setImageWithURLRequest:request placeholderImage:self.defaultImage success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
            if (_enableZoom) {
...
- (void)resetImage:(HMPhotoItem *)photo{
    ...
    if (photo) {
        ...
        [self.imageView setImageWithURLRequest:request placeholderImage:image success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
            [self.imageView setImageWithImage:image];
        } failure:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, NSError * _Nonnull error) {
