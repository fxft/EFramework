...
{
    return [MotionOrientation affineTransformFor:self.interfaceOrientation];
}
...
+ (CGAffineTransform)affineTransformFor:(UIInterfaceOrientation)interfaceOr
{
    ...
    if ( [MotionOrientation sharedInstance].showDebugLog ) {
...
- (void)accelerometerUpdateWithData:(CMAccelerometerData *)accelerometerData error:(NSError *)error
{
    if ( error ) {
        ...
        if (self.handler) {
            self.handler(accelerometerData,0,0,error);
        }
        return;
    }
    ...
    if (self.handler) {
        self.handler(accelerometerData,angle,absoluteZ,error);
    }
